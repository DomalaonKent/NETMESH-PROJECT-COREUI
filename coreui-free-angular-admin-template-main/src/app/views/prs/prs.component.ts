...
import { PrsService, PrsData } from './prs.service';
...
export class PrsComponent implements OnInit {
  tabs: TabDef[] = [
    { key: 'portable', label: 'Portable PRS' },
    { key: 'fb',       label: 'FB PRS'       },
    { key: 'mobile',   label: 'Mobile PRS'   },
  ];
  activeTab: string = 'portable';
  tabIndex: number = 0;

  private allDataByTab: Record<string, PrsData[]> = {
    portable: [],
    fb: [],
    mobile: [],
  };
  ...
  constructor(private router: Router, private prsService: PrsService) {}
  ...
  loadData(): void {
    this.tabs.forEach(tab => {
      this.prsService.getDataByType(tab.key).subscribe({
        next: (data: PrsData[]) => {
