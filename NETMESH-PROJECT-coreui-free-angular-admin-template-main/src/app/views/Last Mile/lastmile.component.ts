...
export class LastMileComponent implements OnInit {
  ...
  selectedItem:     LastMileData | null  = null;
  isSavingForm:     boolean              = false;
  formErrorMessage: string               = '';
  formData:         LastMileFormData     = this.emptyFormData();
  ...
  private emptyFormData(): LastMileFormData {
    return {
      location:             '',
      barangay:             '',
      cityMunicipality:     '',
  ...
  private calcStats(rows: LastMileData[]): ProviderStats {
    ...
    return {
      totalTests:     n,
      avgDownload:    rows.reduce((s, r) => s + this.parseNum(r.averageDownloadSpeed), 0) / n,
      avgUpload:      rows.reduce((s, r) => s + this.parseNum(r.averageUploadSpeed),   0) / n,
  ...
  async uploadFromLocalFile(): Promise<void> {
    ...
    const result = await readExcelFileWithSummary<LastMileData>(file);
  ...
  async uploadFromUrl(): Promise<void> {
    ...
    try {
      ...
      const result      = await readExcelFromUrlWithSummary<LastMileData>(downloadUrl);
      this.allData      = [...result.successRows, ...this.allData];
      this.refreshTable();
      ...
      this.urlErrorMessage = 'Failed to load file from URL. Please check the URL and try again.';
      console.error(error);
    } finally {
      this.isLoadingFromUrl = false;
      this.cdr.detectChanges();
  ...
  openEditForm(item: LastMileData): void {
    ...
    this.formData = {
      ...
      serviceUptime:        item.serviceUptime        ?? '',
  ...
  saveForm(): void {
    ...
    if (this.isEditMode && this.selectedItem) {
      ...
      const newRecord: LastMileData = {
        ...
        averageDownloadSpeed: this.formData.averageDownloadSpeed,
        averageUploadSpeed:   this.formData.averageUploadSpeed,
        latency:              this.formData.latency,
        packetLoss:           this.formData.packetLoss,
        jitter:               this.formData.jitter,
        serviceAvailability:  this.formData.serviceAvailability,
        serviceUptime:        this.formData.serviceUptime,
